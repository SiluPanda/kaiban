...
export function createServer(): McpServer {
  ...
  server.registerTool('list_tasks', {
    ...
    inputSchema: {
      ...
      priority: z.string().optional().describe('Filter by priority (P0-P3)'),
  ...
  server.registerTool('create_task', {
    ...
    inputSchema: {
      ...
      priority: z.string().optional().describe('Priority (P0-P3)'),
      assignee_id: z.string().optional().describe('Assignee UUID'),
      assignee_type: z.string().optional().describe('human or agent'),
      parent_task_id: z.string().optional().describe('Parent task UUID'),
  ...
  server.registerTool('update_task', {
    ...
    inputSchema: {
      ...
      assignee_id: z.string().nullable().optional().describe('Assignee UUID'),
      assignee_type: z.string().optional().describe('Assignee type'),
      labels: z.array(z.string()).optional().describe('Labels'),
      estimate: z.string().nullable().optional().describe('Estimate'),
  ...
  server.registerTool('start_session', {
    description: 'Begin an agent work session',
    inputSchema: { tasks: z.array(z.string()).default([]).describe('Task IDs') },
  }, async ({ tasks }) => ({
    content: [{ type: 'text', text: JSON.stringify({ message: 'Session started', tasks, startedAt: new Date().toISOString() }, null, 2) }],
  }));

  server.registerTool('end_session', {
    description: 'End an agent session with optional summary',
    inputSchema: { summary: z.string().optional().describe('Summary of work done') },
  }, async ({ summary }) => ({
    content: [{ type: 'text', text: JSON.stringify({ message: 'Session ended', summary, endedAt: new Date().toISOString() }, null, 2) }],
  }));
  ...
  server.registerResource('Project Board',
    ...
    async (uri, { slug }) => {
      ...
      return { contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(grouped, null, 2) }] };
    },
  );

  server.registerResource('Project Backlog',
    ...
    async (uri, { slug }) => {
      ...
      return { contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(tasks.data, null, 2) }] };
    },
  );

  server.registerResource('Task Context',
